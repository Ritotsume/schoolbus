<div class="side-body">
  <div class="page-title">
    <span class="title">404 - Não encontrado</span>
    <div class="description">O conteúdo não pode ser encontrado.</div>
  </div>
  <div class="row">
    <div class="col-xs-12">
      <div class="card">
        <div class="card-header">
          <div class="card-title">
            <div class="title">Desculpe. Mas o conteúdo que você procura não foi encontrado.</div>
          </div>
        </div>
        <div class="card-body">
          <h4>Siga esses passos para tentar solucionar o problema:</h4>
          <ul>
            <li>Tente acessar novamente o conteúdo desejado.</li>
            <li>Anote a URL ou caminho utilizado que levou a esta página.</li>
            <li>
              Entre em contato com o suporte para que o caso seja estudado. Lembre-se de emcaminhar as informações
              obtidas no passo anterior, isso facilitará a solução do problema.
            </li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</div>
